...
var AuthorWhere = struct {
	...
	ID:   whereHelperstring{field: "\"author\".\"id\""},
	Name: whereHelperstring{field: "\"author\".\"name\""},
	Age:  whereHelpernull_Int{field: "\"author\".\"age\""},
}
...
var (
	authorAllColumns            = []string{"id", "name", "age"}
	authorColumnsWithoutDefault = []string{"id", "name", "age"}
...
func (authorL) LoadBooks(ctx context.Context, e boil.ContextExecutor, singular bool, maybeAuthor interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`book`),
		qm.WhereIn(`book.author_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *Author) SetBooks(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Book) error {
	query := "update \"book\" set \"author_id\" = null where \"author_id\" = $1"
